...
function TombolTo(
    ...
    return '<a class="' . $Tombolclass . '" href="' . $link . '">' . $valueTombol . '</a>';
}
...
function TombolIDcheck($yesno, $link, $valueinput, $confirmText = "Apakah Anda Yakin?",)
{
    switch ($yesno) {
        case '0':
            return '<form action="' . $link . '" method="POST">' . csrf_field() . '<label>' .
...
function TombolID(
    ...
    return '<form action="' . $link . '" method="POST" class="' . $formclass . '">' .
        csrf_field() .
        // hidden input
